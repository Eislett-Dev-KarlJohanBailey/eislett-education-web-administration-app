...
import { handleFetchQuestionPlanById, handleUpdateQuestionPlan, CreateQuestionPlanRequest, QuestionPlan } from "@/services/questionPlans/questionPlansRequest";
...
export default function EditQuestionPlanPage() {
  ...
  // Fetch question plan by ID
  useEffect(() => {
    if (!router.isReady || !id || !authContext?.token) return;

    const fetchQuestionPlan = async () => {
      setIsLoading(true);
      try {
        const response = await handleFetchQuestionPlanById(authContext.token, id as string);
        ...
        if (response.error) {
          ...
          console.log("Fetched question plan:", plan);
          setOriginalPlan(plan);
          setFormData({
            subTopicId: plan.subTopicId || "",
            prompt: plan.prompt || "",
            bannedList: plan.bannedList || [],
            creativityLevel: plan.creativityLevel ?? 0.7,
            difficultyLevel: plan.difficultyLevel ?? 0.5,
            tags: plan.tags || [],
            quota: plan.quota || 5,
        ...
        console.error("Error fetching question plan:", error);
        toast({
          title: "Error fetching question plan",
          style: { background: "red", color: "white" },
          duration: 3500,
        });
        router.push("/admin/question-plans");
  ...
  const handleFormSubmit = useCallback(async () => {
    ...
    try {
      // Format created array with question IDs from original plan
      const createdArray = (originalPlan?.created || [])
      ...
      const updateData: CreateQuestionPlanRequest = {
        ...
        locked: originalPlan?.locked || false,
      };

      const response = await handleUpdateQuestionPlan(authContext.token, id as string, updateData);
      ...
      if (response.error) {
        ...
        toast({
          title: "Question plan updated successfully!",
      ...
      console.error("Error updating question plan:", error);
      toast({
        title: "Error updating question plan",
        style: { background: "red", color: "white" },
        duration: 3500,
      });
    } finally {
  ...
  }, [authContext?.token, formData, router, id, originalPlan]);
      ...
      <AdminLayout>
        <div className="p-6">
          <div className="flex items-center justify-center py-12">
            <p className="text-muted-foreground">Loading question plan...</p>
          </div>
        </div>
      </AdminLayout>
    );
    ...
    <AdminLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Edit Question Plan</h1>
            <p className="text-muted-foreground">Update AI question generation settings</p>
          </div>
        </div>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button onClick={handleFormSubmit} disabled={isSubmitting}>
                {isSubmitting ? "Updating..." : "Update Question Plan"}
              </Button>
            </div>
          </CardContent>
        </Card>
